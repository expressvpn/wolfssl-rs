...
const PATCHES: &[&str] = &[
    ...
    "wolfssl-namespace-arm-AES_set_encrypt_key.patch",
];
...
fn build_win(wolfssl_src: &Path) -> PathBuf {
    let msb = MsBuild::find_msbuild_in_range(
        Some(msbuild::ProductLineVersion::Vs2026.installation_version_max()),
        Some(msbuild::ProductLineVersion::Vs2022.installation_version_min()),
    )
    .expect("Failed to find MsBuild with required version");
...
fn build_wolfssl(wolfssl_src: &Path) -> PathBuf {
    ...
    if cfg!(feature = "postquantum") {
        ...
        conf.enable("mlkem", Some("yes,all"))
            ...
            .enable("sha3", None)
            // Enable extra PQ/T hybrid combinations (e.g. SECP521R1MLKEM1024)
            .enable("extra-pqc-hybrids", None);
    }
